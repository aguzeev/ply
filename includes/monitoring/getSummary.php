...
if (strpos($field, ',') > 0) {
	...
	$_count_fields = sizeof($fieldsArr);
	$scaleForField = $scale[$machine][$fieldsArr[0]];
...
$sql = "(SELECT " . $fieldString . " FROM `" . $machine . "` WHERE `timestamp` BETWEEN '" . $beg . "' AND '" . $end . "') UNION ALL (SELECT " . $fieldString . " FROM `" . $machine . "_corrections` WHERE `timestamp` BETWEEN '" . $beg . "' AND '" . $end . "')";
...
while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
	if ($_count_fields == 1) {
		...
		for ($i = 0; $i < $_count_fields; $i++) {
			$val_temp[$i] += $row[$fieldsArr[$i]];
			$n[$i]++;
		}
	}
}
...
switch($operation) {
	case 'sum':
		if ($_count_fields == 1) { $valTotal = $val_temp; }
			else { $valTotal = array_sum($val_temp); }
	break;
	case 'sumOnBins':
		if ($_count_fields == 1) { $valTotal = $val_temp; }
			else { $valTotal = array_sum($val_temp); }
	...
	case 'avg':
		if ($_count_fields == 1) {
			...
			if (array_sum($n)) $valTotal = array_sum($val_temp) / array_sum($n);
				else $valTotal = 0;
		}
	break;
	case 'count':
